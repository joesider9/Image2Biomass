...
from eforecast.datasets.image_data.image_dataset_preprocess import ImageDatasetRealTime
...
def collate_fn_eval(batch):
    ...
    x = [item[0] for item in fn]
    y = []
    for item in fn:
        y += item[1]
    dict_batch1 = [item['images'] for item in x]
    dict_batch2 = [item['row_stats'] for item in x]
    collated_dict1 = torch.cat(dict_batch1, 0)
    collated_dict2 = torch.cat(dict_batch2)

    return {'images': collated_dict1, 'row_stats':collated_dict2}, y


def collate_fn_train(batch):
    fn = list(filter(lambda x: x[0] is not None if isinstance(x, tuple) else x is not None, batch))
    x = [item[0] for item in fn]
    y = [item[1] for item in fn]
    dict_batch1 = [item['images'] for item in x]
    dict_batch2 = [item['row_stats'] for item in x]
    collated_dict1 = torch.cat(dict_batch1, 0)
    collated_dict2 = torch.cat(dict_batch2)
    return {'images': collated_dict1, 'row_stats':collated_dict2}, torch.cat(y)
...
def feed_image_dataset_real_time(static_data, data, target, dates, params, device, batch_size=1, shuffle=False,
    ...
    dataset = ImageDatasetRealTime(static_data, data, target, dates, params, train=train, use_target=use_target)
    dataloader = DataLoader(dataset, shuffle=shuffle, batch_size=batch_size, num_workers=16, collate_fn=collate_fn_train
