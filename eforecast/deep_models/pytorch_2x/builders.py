...
class build_graph(nn.Module):
    def __init__(self, x, model_layers, params, is_fuzzy=False, train=True,
        ...
        if not probabilistic:
            ...
            if params['n_out'] > 1:
                for i in range(params['n_out']):
                    layer_output = layers_functions['dense'](shape, {'act_func': None}, 1,
                                                             name_scope,
                                                             f'output_{i}', train=train)
                    self.model_layers_built[f'output_{i}'] = layer_output
                    self.non_input_branch_names.add(f'output_{i}')
            else:
    ...
    def forward(self, x, get_activations=False):
        ...
        name_scope = 'output'
        if self.params['n_out'] == 1:
            return self.model_layers_built[name_scope](outputs['hidden_output'])
        else:
            output = []
            for i in range(self.params['n_out']):
                output.append(self.model_layers_built[f'output_{i}'](outputs['hidden_output']))
