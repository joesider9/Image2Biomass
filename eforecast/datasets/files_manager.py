...
class FilesManager:
    ...
    def split(self, data, dates):
        if self.static_data['Evaluation_start'] is None:
            return data, dates
        if self.static_data['remove_night_hours']:
            try:
                with Pool(self.static_data['n_jobs']) as pool:
        ...
        if self.train:
            if not isinstance(self.static_data['Evaluation_start'], list):
                ...
                if len(self.static_data['Evaluation_start']) == 2:
                    dates_eval = dates[np.where((dates > self.static_data['Evaluation_start'][0]) & (dates < self.static_data['Evaluation_start'][1]))[0]]
