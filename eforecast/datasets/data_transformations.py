        ...
        for transformation in transformations:
            if transformation not in {'inverse', 'brightContrast', 'sum', 'eq_histogram', 'grey', 'sam'}:
                if transformation == 'clear_sky':
                    ...
                    self.transformers[variable][transformation] = self.transformer_params[variable]
    ...
    def transform(self, data, variable, data_dates=None, inverse=False):
        ...
        for transformation in transformations:
            ...
            if transformation == 'clear_sky':
                ...
                trans_param = self.transformers[variable][transformation]
                if not inverse:
                    data[variable] = data[variable].apply(
                        lambda x: [t[0] for t in trans_param if t[1] == x][0])
                else:
                    data[variable] = data[variable].apply(
                        lambda x: [t[1] for t in trans_param if t[0] == x][0])
            elif transformation == 'pv_clear_sky':
                if isinstance(data, pd.Series):
                    data = data.to_frame()
                if isinstance(data, pd.DataFrame):
