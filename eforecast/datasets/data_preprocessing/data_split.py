...
class Splitter:
    ...
    def split_cluster_data(self, clustered_dates, cv_mask):
        ...
        if isinstance(dates[0], pd.Timedelta):
            ...
            cluster_cvs = dict()
            for rule_name, rule_dates in clustered_dates.items():
                n_total = len(rule_dates)
                n_sample = int(n_total * self.val_test_ratio)
                sample_positions = np.random.choice(n_total, n_sample, replace=False)
                dates_val = rule_dates[sample_positions]
                rule_dates = rule_dates.drop(dates_val)
                n_total = len(rule_dates)
                n_sample = int(n_total * self.val_test_ratio)
                sample_positions = np.random.choice(n_total, n_sample, replace=False)
                dates_test = rule_dates[sample_positions]
                dates_train = rule_dates.drop(dates_test)
                cluster_cvs[rule_name] = [dates_train, dates_val, dates_test]
        return cluster_cvs
    ...
    def split(self, refit=False):
        ...
        if cv_mask is None:
            _, data_row = self.file_manager.check_if_exists_image_data()
            tag = 'colour_ratio'
            data_row = data_row[tag].dropna(axis='index')
            if isinstance(data_row.index, pd.TimedeltaIndex):
                if data_row is None:
                ...
                df = data_row.copy()
                mask_val = df.sample(frac=self.val_test_ratio).index
                df = df.drop(mask_val)
                mask_test = df.sample(frac=self.val_test_ratio).index
                mask_train = df.drop(mask_test).index
