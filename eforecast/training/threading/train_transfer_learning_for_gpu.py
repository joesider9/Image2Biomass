...
def GPU_thread(static_data, static_data_base, n_gpus, n_jobs, cluster=None, method=None, refit=False, gpu_id=None):
    clusters = joblib.load(os.path.join(static_data['path_model'], 'clusters.pickle'))

    res = []
    if gpu_id is not None:
        n_gpus = 1
        gpu_ids = {cluster_name: gpu_id for i, cluster_name in enumerate(clusters.keys())}
    else:
...
def train_transfer_learning_on_gpus(static_data, static_data_base, refit=False, gpu_id=None, n_jobs=None):
    ...
    for method in global_methods:
        ...
        GPU_thread(static_data, static_data_base, n_gpus, static_data[method]['n_jobs'] if n_jobs is None else n_jobs,
                   cluster=clusters, method=method, refit=refit, gpu_id=gpu_id)
