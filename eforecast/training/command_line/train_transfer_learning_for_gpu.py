...
def GPU_thread(static_data, n_gpus, n_jobs, method, clusters, runtime_file, static_data_base, refit=False, gpu_id=None):
        ...
        if not os.path.exists(os.path.join(cluster_dir, f'results_{cluster_name}_{method}.csv')) or refit:
            ...
            for experiment in os.listdir(os.path.join(clusters_base[cluster_name], method)):
                ...
                task = {'trial_number': trial_number, 'method': method,
                        ...
                        'gpu_id': i % n_gpus if gpu_id is None else gpu_id,
                        'job_id': j % n_jobs,
                        'refit': refit_trial}
                tasks.append(task)
                j += 1
...
def train_transfer_learning_on_gpus(static_data, static_data_base, refit=False, gpu_id=None, n_jobs=None):
    ...
    for method in global_methods:
        ...
        GPU_thread(static_data, n_gpus, static_data[method]['n_jobs'] if n_jobs is None else n_jobs, method,
                   clusters, runtime_file, static_data_base, refit=refit, gpu_id=gpu_id)
