from configuration.config_utils import define_n_jobs
from configuration.config_utils import define_enviroment
from configuration.config_project import TYPE
from configuration.config_project import HORIZON_TYPE
from configuration.config_input_data import NWP_DATA_MERGE
from configuration.config_input_data import DATA_COMPRESS
from configuration.config_input_data import DATA_ROW_SCALE
from configuration.config_input_data import DATA_NWP_SCALE
from configuration.config_input_data import DATA_IMG_SCALE
from configuration.config_input_data import DATA_IMG
from configuration.experiments import experiments

RUNTIME_BACKEND = 'TORCH'  # TF_2, TORCH

from configuration.timm_model_list import timm_models
from configuration.huggingface_vit_list import huggingface_models
...
def rbf_variables():
    if TYPE == 'pv':
        ...
        var_imp = [['img_masked_H_95', 'img_masked_S_95', 'img_masked_V_95']]
    else:
        var_imp = [[]]
    return var_imp
...
def config_methods():
    ...
    static_data['TIMM'] = {'experiment_tag': 'timm_net5',
                           'n_trials': 2,
                           'model_name': ['visformer_small', 'Visual-Attention-Network/van-tiny'],
                           'timm_models': ' '.join(timm_models),
    ...
    static_data['MLP'] = {'experiment_tag': {'mlp1', 'mlp2'},
                          'n_trials': 15,
                          'hold_prob': 1,
                          'batch_size': 128,
                          'transfer_learning_from_global': False,
    ...
    static_data['CatBoost'] = {'n_trials': 20,
                               'iterations': 1000,
                               'scale_img_method': set(DATA_IMG_SCALE),
                               'image_data_type': set(DATA_IMG),
                               ...
                               'min_data_in_leaf': [9, 30],
                               "colsample_bylevel": [0.7, 0.97],
                               "depth": [5, 9],
                               "boosting_type": "Ordered",
                               "bootstrap_type":  "MVS",
                               "eval_metric": "MAE" if HORIZON_TYPE != 'multi-output' else 'MultiRMSE',
                               }
    static_data['lightgbm'] = {'n_trials': 40,
